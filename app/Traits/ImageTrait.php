...
trait ImageTrait
{
    ...
    public function saveImageHttp($requestImage, $for = '_product_', $save_to_db = false,$url=null,$token_id=null)
    {
        ...
        if ((!empty($requestImage) && $requestImage != 'null') || $url) :
            ...
            if (!$url) {
                ...
                if ($requestImage instanceof UploadedFile) {
                    ...
                    // Assuming $requestImage is a URL, you can fetch its extension and mime type here.
                    $extension = pathinfo($requestImage, PATHINFO_EXTENSION);
                    // dd($extension);
                    // $mime_type = mime_content_type($requestImage);
                }
            }
            ...
            elseif ($for == 'admin_light_logo' || $for == 'admin_dark_logo' || $for == 'footer_logo' || $for == 'invoice_logo' ||
                ...
                elseif ($for == 'light_logo' || $for == 'dark_logo'):
                    $image_138x52       = date('YmdHis') . "-" . $for . '-138x52' . rand(1, 500) . '.png';
            ...
            elseif ($for == 'single_file'):
                ...
                $image_110x122          = date('YmdHis') . "image_110x122" . $for . rand(1, 500) . '.' . $extension;
                $image_320x320          = date('YmdHis') . "image_320x320" . $for . rand(1, 500) . '.' . $extension;
                $image_387x280          = date('YmdHis') . "image_387x280" . $for . rand(1, 500) . '.' . $extension;
